...
jQuery.fn.floatdiv = function (location) {
    ...
    function initPosition() {
        var showPostion = $(that).parent().position();
        return { top: parseInt(showPostion.top + 10) + "px", left: parseInt(showPostion.left + 2) + "px" };
    };
    $(window).resize(function () {
        $(that).floatdiv();
    });
    return this.each(function () {
        location = $.extend(initPosition(),location);
        var loc; //层的绝对定位位置
